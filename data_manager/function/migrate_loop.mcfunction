...
function data_manager:migrate_copy with storage central:temp migrate

# Increment and loop
scoreboard players add $migrate.index temp 1
execute if score $migrate.index temp < $migrate.count temp run function data_manager:migrate_loop
